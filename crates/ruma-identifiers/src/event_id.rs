...
/// // Original format
/// assert_eq!(
///     Box::<EventId>::try_from("$h29iv0s8:example.com").unwrap().as_ref(),
///     "$h29iv0s8:example.com"
/// );
/// // Room version 3 format
/// assert_eq!(
///     Box::<EventId>::try_from("$acR1l0raoZnm60CBwAVgqbZqoO/mYU81xysh1u7XcJk")
///         .unwrap()
///         .as_ref(),
...
/// assert_eq!(
///     Box::<EventId>::try_from("$Rqnc-F-dvnEYJTyHq_iKxU2bZ1CI92-kuZq3a5lr5Zg")
///         .unwrap()
///         .as_ref(),
...
impl EventId {
    /// Generates an `EventId` for the given origin server with a localpart consisting of 18 random
    /// ASCII characters.
    ///
    /// This should only be used for events in the original format as used by Matrix room versions 1
    /// and 2.
    #[cfg(feature = "rand")]
    ...
    /// "localpart" that precedes the homeserver. For later formats, this is the entire ID
...
mod tests {
    ...
    fn valid_original_event_id() {
        assert_eq!(
            Box::<EventId>::try_from("$39hvsi03hlne:example.com")
    ...
    fn valid_original_event_id_with_explicit_standard_port() {
        assert_eq!(
            Box::<EventId>::try_from("$39hvsi03hlne:example.com:443")
                .expect("Failed to create EventId."),
    ...
    fn valid_original_event_id_with_non_standard_port() {
        assert_eq!(
            Box::<EventId>::try_from("$39hvsi03hlne:example.com:5000")
                .expect("Failed to create EventId."),
    ...
    fn missing_original_event_id_sigil() {
        assert_eq!(
            Box::<EventId>::try_from("39hvsi03hlne:example.com").unwrap_err(),
    ...
    fn missing_base64_event_id_sigil() {
        assert_eq!(
            Box::<EventId>::try_from("acR1l0raoZnm60CBwAVgqbZqoO/mYU81xysh1u7XcJk").unwrap_err(),
            Error::MissingLeadingSigil
        );
    }
    ...
    fn missing_url_safe_base64_event_id_sigil() {
        assert_eq!(
            Box::<EventId>::try_from("Rqnc-F-dvnEYJTyHq_iKxU2bZ1CI92-kuZq3a5lr5Zg").unwrap_err(),
            Error::MissingLeadingSigil
        );
    }
    ...
    fn invalid_event_id_host() {
        assert_eq!(
            Box::<EventId>::try_from("$39hvsi03hlne:/").unwrap_err(),
    ...
    fn invalid_event_id_port() {
        assert_eq!(
            Box::<EventId>::try_from("$39hvsi03hlne:example.com:notaport").unwrap_err(),
            Error::InvalidServerName
        );
    }
}
